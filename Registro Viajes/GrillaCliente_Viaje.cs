...
{
    public partial class GrillaCliente_Viaje : Form
    {

        public AltaViaje formularioAlta;

        public GrillaCliente_Viaje(AltaViaje formulario)
        {
            InitializeComponent();
            ...
            if (senderGrid.Columns[e.ColumnIndex] is DataGridViewButtonColumn && senderGrid.CurrentCell.Value.ToString() == "Seleccionar" && e.RowIndex >= 0)
            {
                try
                {
                    if ((Byte)senderGrid.CurrentRow.Cells["Cliente_Activo"].Value == 1)
                    {
                        ...
                        this.formularioAlta.clienteElegido = clienteSeleccionado;
                        this.formularioAlta.cambiarCliente();
                        this.Hide();
                    }
                    else
                    {
